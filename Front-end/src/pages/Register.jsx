...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!passwordRegex.test(password)) {
      return setError('Password must be at least 6 characters and include at least one uppercase letter, one lowercase letter, one number, and one special character (@$!%*?&).');
    }
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12 sm:px-6 lg:px-8 relative">
      {/* Glow effects */}
      <div className="absolute top-1/2 left-1/2 -z-10 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-secondary/10 blur-3xl opacity-30"></div>
      ...
      <div className="w-full max-w-md space-y-8 glass-panel p-8 sm:p-10 rounded-2xl border border-white/5 shadow-2xl relative">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Join ResumeAI to analyze your first resume
          </p>
        </div>
